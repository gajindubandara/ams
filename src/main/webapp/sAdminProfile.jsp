...
<title>The Jobs | Super Admin Profile</title>
</head>
<body>
...
<!-- md5 hashing -->
	...
	<script>
	var user = JSON.parse(sessionStorage.getItem("superAdmin"));
	console.log(user);
	...
	saveButton.addEventListener('click', () => {
		...
		if (!name || !email || !number ) {
			...
			if (name === responseName && email === responseEmail && number === responseNumber) {
				Swal.fire({
					icon: 'error',
					...
					success: function(response) {
						...
						Swal.fire({
							...
							if (result.isConfirmed) {
								window.location.href = 'sAdminProfile.jsp';
							}
						});
